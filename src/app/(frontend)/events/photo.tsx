...
export default function Gallery({ customLimit = 0 }: { customLimit?: number }) {
  ...
  const { data, error, isLoading } = useSWR(
    `http://localhost:3000/api/v1/admin/gallery/list?customLimit=${customLimit}`,
    fetcher
  );
  useEffect(() => {
    setHasMounted(true);
                      ...
                      {(customLimit > 0 ? currentVideos.slice(0, customLimit) : currentVideos).map((video, index) => (
                        ...
                        >
                          <div className="relative aspect-video bg-gray-200 dark:bg-gray-700 rounded-lg mb-4 overflow-hidden hover:border-4 hover:border-orange-600">
                            <iframe
                              className="w-full h-full object-cover"
                              src={`${video.url}${video.url.includes('?') ? '&' : '?'}modestbranding=1`}
                              title="YouTube video"
                            ...
                            <div className="absolute bottom-2 right-2 bg-orange-600 text-white text-xs px-2 py-1 rounded">
                              4:43
                            </div>
                          </div>
