...
export default function EventsListTable({ initialData }: Props) {
    const [allGalleries, setAllGalleries] = useState<Evnets[]>(initialData);
    ...
    const fetchAllGalleries = useCallback(async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/v1/admin/events/list/?perPage=${pageSize}`, {
                credentials: 'include',
            });
            const result: EventsApiResponse = await response.json();
            if (result.success && result.data) {
                setAllGalleries(result.data.events || []);
            } else {
    ...
    const changeStatus = async (galleryId: string, currentStatus: boolean) => {
        try {
            ...
            const response = await fetch(`/api/v1/admin/events/update/${galleryId}`, {
                method: 'PATCH',
                credentials: 'include',
                body: formData,
            });
    ...
    const handleEdit = (galleryId: string) => {
        router.push(`/admin/events/add?id=${galleryId}`);
    };
